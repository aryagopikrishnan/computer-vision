...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if start_multiple in range(start,stop):
      x = torch.arange(((start+9) // 10)*10, stop, 10, dtype=torch.float64)
    else:
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    #print(min_idx)
    #print(x.shape[0])
    #y[range(x.shape[0], min_idx)] = 0
    y[range(y.size(0)), min_idx] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    #dim=0 indicates column
    mean_column = x.sum(dim=0) / x.size(0)
    std_dev = torch.sqrt(((x-mean_column)**2).sum(dim=0)/(x.size(0)-1))
    y = (x - mean_column)/std_dev 
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    xs_list = torch.stack(xs)
    a = torch.arange(xs_list.size(1)).reshape(1,-1) < ls.reshape(-1,1)
    y = xs_list[a].view(len(xs), -1).mean(dim=1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    uniques, indices = torch.unique(x, return_inverse=True, return_counts=False)
